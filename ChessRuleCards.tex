...
\begin{center}
    {\captionfontsize \textsf{\textbf{CAMOUFLAGE}}}\end{center}
        ...
        \tikz{\fill (0,0) rectangle (\cardwidth-2*\strippadding-2*\textpadding,\ruleheight);}\\
        {\quotefontsize \textit{\footnotesize Camouflage is a game we all like to play, but our secrets are as surely revealed by what we want to seem to be as by what we want to conceal. ---Russell Lynes}}\\[-2\baselineskip]
    };
    \node[circle,draw,text=black](c) at (.5,\cardheight-.5){C};
    \end{tikzpicture}%
\begin{tikzpicture}
