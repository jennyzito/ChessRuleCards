...
def lava(x,y,square=10):
    red = sd.square(2*square)
    red = sd.translate([3*square,3*square])(red)
    return red
...
if __name__ == '__main__':
    ...
    svg_lava = scad2svg(lava(x,y,square))
    svg_lava = setSVGproperty(svg_lava, fill='red', stroke_width=0)
    svg_back = joinSCAD_SVG(svg_bb, svg_back)
    total = joinSCAD_SVG(svg_back, svg_board)
    total = joinSCAD_SVG(total, svg_lava)
    print(total)
