...
if __name__ == '__main__':
    square = 10
    r=4/np.pi*square
    block = sd.square([2*r,8*square], center=True)
    end = sd.circle(r=r)
    end = sd.scale([1,.5])(end)
    block += sd.translate([0,-4*square])(end)
    back = block + sd.translate([0,4*square])(end)
    block -= sd.translate([0,4*square])(end)
    band = block - sd.translate([0,square])(block)
    Band = [sd.translate([0,i*square])(band) for i in range(8)]
    col_in = sd.square([r/np.sqrt(2),20*square])
    col_in = sd.translate([0,-10*square])(col_in)
    col_out = sd.translate([r/np.sqrt(2),0])(col_in)
    total = sd.union()(*[sd.intersection()(Band[i],col_in) for i in range(0,8,2)])
    total += sd.union()(*[sd.intersection()(Band[i],col_out) for i in range(1,8,2)])
    total += sd.union()(*[sd.intersection()(Band[i],sd.translate([-r/np.sqrt(2),0])(col_in)) for i in range(1,8,2)])
    total += sd.union()(*[sd.intersection()(Band[i],sd.translate([-2*r/np.sqrt(2),0])(col_in)) for i in range(0,8,2)])
    svg_cyl = scad2svg(total)
    svg_cyl = setSVGproperty(svg_cyl, fill='black', stroke_width=0)
    svg_top = scad2svg(sd.translate([0,4*square])(end))
    svg_top = setSVGproperty(svg_top, fill='white', stroke_width=.5)
    svg_back = scad2svg(back)
    svg_back = setSVGproperty(svg_back, fill='white', stroke_width=.5)
    # print(svg_cyl)
    ...
    total = joinSCAD_SVG(svg_back, svg_top)
    total = joinSCAD_SVG(total, svg_cyl)
    print(total)
